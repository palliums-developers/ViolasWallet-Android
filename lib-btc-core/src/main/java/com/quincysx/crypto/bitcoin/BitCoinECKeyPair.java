...
public class BitCoinECKeyPair extends ECKeyPair {
    public static final int TEST_NET_ADDRESS_SUFFIX = 0x6f;
    public static final int MAIN_NET_ADDRESS_SUFFIX = 0x00;

    public static final int MAIN_NET_PRIVATE_KEY_PREFIX = 0x80;
    public static final int TEST_NET_PRIVATE_KEY_PREFIX = 0xef;
    protected static final int MAIN_NET_PRIVATE_KEY_SUFFIX = 0x01;

    protected static final int RAW_PRIVATE_KEY_COMPRESSED_LENGTH = 38;
    protected static final int RAW_PRIVATE_KEY_NO_COMPRESSED_LENGTH = 37;

    protected final boolean testNet;
    ...
    public BitCoinECKeyPair(byte[] priv, byte[] pub, boolean testNet) throws
            ValidationException {
        super(priv, pub);
        this.testNet = testNet;
    }
    ...
    public byte[] sign(BTCTransaction btcTransaction) {
        byte[] signature = BitCoinECKeyPair.sign(priv, Script
                .hashTransactionForSigning(btcTransaction));
        ...
        signatureAndHashType[signatureAndHashType.length - 1] = Script.SIGHASH_ALL;
    ...
    public static BTCTransaction.Input sign(BTCTransaction transaction, int index, BigInteger
        ...
        //双 Sha256 and 签名
        byte[] signature = sign(privateKeyBigInteger, Script
        ...
        //拼出新的input
        return new BTCTransaction.Input(transaction.inputs[index].outPoint, new Script(signatureAndHashType, publicKey), transaction.inputs[index].sequence);
    }
    ...
    private static BTCTransaction signatureForm(BTCTransaction transaction, int index, String
        ...
        for (int i = 0; i < transaction.inputs.length; i++) {
            ...
            if (i == index) {
                //设置地址签名
                script = new Script(HexUtils.fromHex(publicScript));
