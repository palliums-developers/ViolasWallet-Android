package org.palliums.violascore.wallet;

const val MNEMONIC_SALT_DEFAULT = "LIBRA"
const val DERIVED_KEY = "LIBRA WALLET: derived key$"
const val LibraHashSuffix = "@@$\$LIBRA$$@@"
const val MASTER_KEY_SALT = "LIBRA WALLET: master key salt$"
const val MNEMONIC_SALT_PREFIX = "LIBRA WALLET: mnemonic salt prefix$"
const val RAW_TRANSACTION_HASH_SALT = "RawTransaction::libra_types::transaction${org.palliums.libracore.wallet.LibraHashSuffix}"