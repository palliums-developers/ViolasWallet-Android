...
class ApprovalFragmentViewModel(
    ...
    companion object {
        ...
        const val ACTION_APPROVE_APPLICATION = 0x03             // 审核通过发币申请
        const val ACTION_TRANSFER_COIN_TO_ISSUER = 0x04         // 转平台币给发行商
        const val ACTION_MINT_TOKEN_TO_ISSUER = 0x05            // 铸稳定币给发行商
    }

    val mUnapproveReasonsLD by lazy {
        MutableLiveData<List<UnapproveReasonDTO>>()
    }
    val mIsTransferredCoinToIssuerLD by lazy {
    ...
    init {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (mSSOApplicationDetails.applicationStatus == SSOApplicationState.CHAIRMAN_APPROVED) {
                val transferred = mGovernorManager.isTransferredCoinToIssuer(
                    walletAddress = currentAccount.address,
                    applicationId = mSSOApplicationDetails.applicationId
                )
                mIsTransferredCoinToIssuerLD.postValue(transferred)
            }
        }
    }
    ...
    override suspend fun realExecute(action: Int, vararg params: Any) {
        when (action) {
            ...
            ACTION_TRANSFER_COIN_TO_ISSUER -> {
                mGovernorManager.transferCoinToIssuer(
                    mSSOApplicationDetails,
                    null,
            ...
            ACTION_MINT_TOKEN_TO_ISSUER -> {
                mGovernorManager.mintTokenToIssuer(
    ...
    fun transferCoinToSSO(fragment: Fragment, requestCode: Int) {
        ...
        TransferActivity.start(
            ...
            address = mSSOApplicationDetails.issuerWalletAddress,
            amount = 2 * 1000_000,
            modifyable = false,
